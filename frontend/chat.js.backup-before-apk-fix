...
/* =========================================================
   GAPINO_MASTER_MESSAGE_ACTIONS_V2
...
(function () {
    ...
    if (window.GAPINO_MASTER_MESSAGE_ACTIONS_V2) {
        return;
    }

    window.GAPINO_MASTER_MESSAGE_ACTIONS_V2 = true;
    ...
    function createButton(label, title, className, callback) {
        const button = document.createElement("button");

        button.type = "button";
        button.className = className || "gapino-master-action";
        ...
        button.addEventListener("click", function (event) {
            ...
            try {
                ...
                console.error("[GAPINO ACTION ERROR]", error);
            }
        });
    ...
    function startReply(message) {
        ...
        window.GAPINO_REPLY_MESSAGE = selectedReplyMessage;
        ...
        const oldPreview = document.getElementById(
            "gapinoMasterReplyPreview"
        );
        ...
        const form = input.closest(".message-form");
        ...
        preview.id = "gapinoMasterReplyPreview";
        preview.className = "gapino-master-reply-preview";

        const icon = document.createElement("span");
        icon.className = "gapino-master-reply-icon";
        icon.textContent = "↩";

        const content = document.createElement("div");
        content.className = "gapino-master-reply-content";

        const title = document.createElement("strong");
        title.textContent =
            "پاسخ به " +
            selectedReplyMessage.sender_name;

        const text = document.createElement("span");
        text.textContent =
            selectedReplyMessage.text || "پیام";
        ...
        const cancel = document.createElement("button");
        cancel.type = "button";
        cancel.className = "gapino-master-reply-cancel";
        cancel.textContent = "×";
        cancel.title = "لغو پاسخ";
    ...
    function startEdit(message) {
        ...
        if (
            ...
            showMessageToast(
                "فقط پیام‌های خودت را می‌توانی ویرایش کنی."
            );
            return;
        }
        ...
        if (!text) {
            showMessageToast(
                "این پیام قابل ویرایش نیست."
            );
            return;
        }

        selectedEditMessageId = String(message.id);

        window.GAPINO_EDIT_MESSAGE_ID =
        ...
        if (form && form.parentElement) {
            ...
            preview.id = "gapinoMasterEditPreview";
            preview.className = "gapino-master-edit-preview";

            const title = document.createElement("strong");
            title.textContent = "✏️ در حال ویرایش پیام";

            const cancel = document.createElement("button");
            cancel.type = "button";
            cancel.textContent = "لغو";
            cancel.className = "gapino-master-edit-cancel";
    ...
    function cancelEdit() {
        ...
        window.GAPINO_EDIT_MESSAGE_ID = null;
        ...
        if (input) {
            ...
            try {
                if (typeof resizeMessageInput === "function") {
                    resizeMessageInput();
                }
            } catch (_) {}
        }
    ...
    function renderDeleted(messageId) {
        const row = findRow(messageId);
        ...
        row.classList.add(
            "gapino-master-message-deleted"
        );

        const actions = row.querySelector(
            ".gapino-master-actions"
        );
    ...
    function renderEdited(message) {
        ...
        let textElement = bubble.querySelector(
            ".gapino-master-message-text"
        );
        ...
        if (!textElement) {
            ...
            const time = bubble.querySelector(
                ".message-time"
            );
        ...
        let edited = bubble.querySelector(
            ".gapino-master-edited"
        );
        ...
        if (!edited) {
            ...
            textElement.appendChild(edited);
        }
    }

    function addActions(row, message) {
        if (!row || !message) {
            return;
        }
    ...
    function decorateLatestMessage(message) {
        const box = getMessagesBox();
        ...
        const rows =
            box.querySelectorAll(".message-row");
        ...
        addActions(row, message);
    }

    function findMessageInCurrentChat(messageId) {
        const box = getMessagesBox();
    ...
    const previousAppendMessage =
        typeof window.appendMessage === "function"
            ? window.appendMessage
            : null;

    if (previousAppendMessage) {
        window.appendMessage =
            function (message, scroll) {
                const result =
                    previousAppendMessage(
                        message,
                        scroll
                    );
    ...
    window.GAPINO_START_EDIT =
        startEdit;

    window.GAPINO_CANCEL_EDIT =
        cancelEdit;

    window.GAPINO_DELETE_MESSAGE =
        deleteMessage;

    window.GAPINO_FORWARD_MESSAGE =
        forwardMessage;

    window.GAPINO_RENDER_DELETED =
        renderDeleted;

    window.GAPINO_RENDER_EDITED =
        renderEdited;

    window.GAPINO_FIND_MESSAGE =
        findMessageInCurrentChat;

    window.addEventListener(
        "gapino-master-delete",
        function (event) {
            const id =
                event.detail?.message_id ||
                event.detail?.id;
...
/* =========================================================
   GAPINO_MASTER_SEND_TEXT_V2
   ========================================================= */

(function () {
    "use strict";

    if (window.GAPINO_MASTER_SEND_TEXT_V2) {
        return;
    }

    window.GAPINO_MASTER_SEND_TEXT_V2 = true;
